...
#define RTO 500   //Retransmission time out value: 500 ms
...
main(int argc, char* argv[])
{
    ...
    while(1){
	    ...
	    //Timer is expired
	    if(currentTimeMs() - start >= RTO) {
	    	// Timeout
    		cout << "** Timer expired!" << endl;
    		if(SYNACK) {
				start = currentTimeMs();   // Reset timer
				cout<<"Sending packet 0    Retransmission   SYN"<<endl;
				sendto(sockfd, header, HEADER_LEN, 0, ( sockaddr*)&serverAddr, sizeof(serverAddr));
				countSYN++;
    		} else if(FINACK) {
    			start = currentTimeMs();   // Reset timer
    			cout<<"Sending packet 0    Retransmission   FIN"<<endl;
    			sendto(sockfd, header, HEADER_LEN, 0, ( sockaddr*)&serverAddr, sizeof(serverAddr));
    		} else if(countPkt == 0) {
    			// ACK about the server's SYN
    			sendto(sockfd, header,HEADER_LEN, 0, (sockaddr*)&serverAddr, sizeof(serverAddr));
	            printf("%s\n", "Connection Established again");
	            printf("Sending packet %lu Retransmission\n", ClientACK);
	        ...
	        if(seqnum == savedACK || SYNACK || FINACK) {
	            if(receiveACK.SYN() & receiveACK.ACK()) {  // ASF = 110
	                ...
	                if(!Retransmission){        //when Retransmission is false
			            ...
			            seqnum += recvlen-8;  // should be incremented by the size of data           
            	...
            	sendto(sockfd, header,HEADER_LEN, 0, (sockaddr*)&serverAddr, sizeof(serverAddr));
            	start = currentTimeMs();
            	std::cout<<"Sending packet "<< ClientACK<<"      Retransmission"<< endl;  //???to discuss
            }
