...
void closeConnection(unsigned long filesize, unsigned long myInitialSeq, int sockfd, sockaddr_in clientAddr,unsigned long& cwnd, unsigned long& ssthresh);
void sendFile(FILE* fp, unsigned long filesize, unsigned long myInitialSeq, unsigned short wdSize, int sockfd, sockaddr_in clientAddr, unsigned long& cwnd, unsigned long& ssthresh);
...
int main(int argc, const char **argv)
{
  ...
  while(1)
    ...
    recvlen = recvfrom(sockfd, buf, BUFSIZE, 0, (struct sockaddr *)&clientAddr, &addrlen);
        ...
        std::cout << endl << "===> connection from: " << ipstr << ":" <<ntohs(clientAddr.sin_port)
            <<" "<< clientAddr.sin_addr.s_addr <<std::endl;
            buf[recvlen] = 0;
            TCPHeader FirstCall;
            FirstCall.consume(buf);
            if(FirstCall.flags() != 2)
                {cout << "Received Non-SYN signal! Going back to listening" << endl;cout << FirstCall.flags() << endl;continue;}
            else
                {
                    ...
                    sendto(sockfd, headerToSend, 8, 0, (struct sockaddr *)&clientAddr, sizeof(clientAddr));
                    ...
                    if(recvSA.flags() == 4 && recvSA.seqNum() == clientSeq && recvSA.ackNum() == mySeq)
                        {
                            ...
                            //send file
                            sendFile(fp, filesize, mySeq, recvSA.winSize(), sockfd, clientAddr, cwnd, ssthresh);
                            //reset the offset
                            fseek(fp,0,SEEK_SET);
                            //close the connection
                            closeConnection(filesize, mySeq, sockfd, clientAddr, cwnd, ssthresh);
                        ...
                        cout << "Handshake failed! Going back to listening" << endl;
                }
    }
        else
...
void sendFile(FILE* fp, unsigned long filesize, unsigned long myInitialSeq, unsigned short wdSize, int sockfd, sockaddr_in clientAddr, unsigned long& cwnd, unsigned long& ssthresh)
{
    ...
    while(!allSent || !buffer.empty())//repeatedly send and receive
    {
        ...
        for(int i = nextToSend; ((i < (nextToSend+numToSend)) && !allSent); i++)
        {
            ...
            sendto(sockfd, packet, pktSize[i], 0, (struct sockaddr *)&clientAddr, sizeof(clientAddr));
...
void closeConnection(unsigned long filesize, unsigned long myInitialSeq, int sockfd, sockaddr_in clientAddr, unsigned long& cwnd, unsigned long& ssthresh)
{
    ...
    sendto(sockfd, headerToSend, 8, 0, (struct sockaddr *)&clientAddr, sizeof(clientAddr));//send FIN
    ...
    while(1)//try to receive FIN-ACK
    {
        ...
        if(recvlen == -1)
            ...
            if(currentTimeMs() - time0 >= RTO)
            {
                ...
                sendto(sockfd, headerToSend, 8, 0, (struct sockaddr *)&clientAddr, sizeof(clientAddr));//retransmit FIN
